...
const cicB2BAccountDrift: SectionScenario = {
  ...
  subtitle: 'Lloyds Banking Group plc · 280-branch account · NPS down 12pt · two open Sev-1 cases · QBR overdue. CIC surfaces drift before churn signal lands. (Tier-1 enterprise pilot cohort, not the full 600-site estate.)',
  durationSec: 24,
  events: [
...
const digAttributionRebalance: SectionScenario = {
  ...
  subtitle: 'Bayesian MMM detects over-spend on paid social → reallocates £1.8M of quarterly spend → ROAS 5.1× → 5.5× (+0.4×).',
  durationSec: 18,
  events: [
    ...
    { ...e(8, 'act-care', 'Spend rebalance pushed to ad-buying APIs · paid social £1.8M/qtr → retargeting/RCS/Email', 'Care', 'success'), focus: { route: '/digital/marketing/funnel', target: 'page' } },
    e(11, 'log', 'Adobe AEP + Salesforce MC budgets re-issued · creative re-pacing within 30 min', 'Care'),
    e(13, 'log', 'Forecast: portfolio ROAS 5.1× → 5.5× · incremental revenue +£820k/mo', 'Decisioning', 'success'),
    ...
    { ...e(17, 'resolve', 'Spend rebalanced · ROAS 5.1× → 5.5× · gold.spend_ledger + gold.revenue_attribution updated', 'Network', 'success'), focus: { route: '/digital/marketing/funnel', target: 'page' } },
  ],
};
...
const bssCrossSellFired: SectionScenario = {
  ...
  sectionId: 'bss',
  title: 'Cross-sell · 12.4k Disney+ attaches · £750k margin-share ARR',
  subtitle: 'cross_sell_propensity_v2 surfaces Disney+ NBA · 12.4k attaches × £5.04 retail / £0.42 SnowTelco margin share = £750k margin ARR (£5.0M retail GMV); partner co-marketing share of margin baked in.',
  durationSec: 16,
  events: [
...
const bssAcquisitionFraud: SectionScenario = {
  ...
  subtitle: 'Experian + D&B + Stripe Radar fusion catches synthetic-identity fraud at sign-up · 38 of 1,420 daily applications blocked before SIM activation · saves £52k bad-debt + device-finance loss (avg £1.4k per blocked attempt).',
  durationSec: 20,
  events: [
    ...
    e(13, 'log', 'Avoided bad debt: 38 × (£42 first month + £1,360 device-finance liability + £42 SIM/CAC) = £52k saved · 14 boundary cases passed step-up', 'Decisioning'),
...
const ossB2BFastOrder: SectionScenario = {
  ...
  sectionId: 'oss',
  title: 'TMF 622 · Lloyds Banking Group regional pilot · 280 sites fast-order',
...
const ossCabRollback: SectionScenario = {
  ...
  subtitle: 'A NORMAL CHG (CAB-classified, manual approval) fails on push (RRC config v124 regression on Ericsson cluster). Time Travel restores the config-publication record in Snowflake; ENM picks up v123 on next 60s sync; eNBs converge in 4m. Auto-PIR drafted in 30m for Ofcom GC A3.',
  durationSec: 36,
  events: [
    ...
    e(4, 'plan', 'Plan: cab_rollback_v1 fires · Time Travel restores config-publication table to v123 · ENM polls Snowflake every 60s for new package · freeze further changes · ECAB convened', 'Decisioning'),
    { ...e(6, 'act-snow', 'Time Travel restores Snowflake config table to v123 · ENM consumes rollback record on next 60s sync · eNB RRC converges · KPI recovers in 4m', 'Activation', 'success'), focus: { route: '/oss/change', target: 'page' } },
...
const ossVendorSlaBreach: SectionScenario = {
  ...
  subtitle: 'Field-side: ftf_predict_v1 + vendor_perf_v2 flag risk. Vendor-B reserve crew dispatched in parallel; Vendor-A breaches by 18m. Vendor scorecard auto-debited £24k (per MSA non-customer-impacting tier); 0 customer SLA-credit triggered. (NOC vendor-escalation desk handles vendor case independently.)',
  durationSec: 16,
  events: [
    ...
    e(9, 'log', 'Vendor penalty £24k debited from Vendor-A (per MSA non-customer-impacting tier) · gold.partner_settlements + gold.vendor_perf updated', 'Billing'),
...
const ossSliceActivation: SectionScenario = {
  ...
  subtitle: 'TMF 641 + 3GPP TS 28.531 slice instantiation · Barclays trading-floor URLLC slice (< 5ms latency, 99.999% SLA) · CSMF → NSMF → NSSMF orchestration · NSSF + AMF policy binding · reserve resources across 14 cells.',
  durationSec: 24,
  events: [
    ...
    e(4, 'log', 'Footprint: 14 cells in EC2 · 2 gNBs · 1 UPF instance reserved · CSMF translates SLA → NSMF service profile → NSSMF subnet template · NSSF policy + AMF NSSAI binding generated · MEC node placement at LDN-MEC-01', 'Decisioning'),
    ...
    e(8, 'plan', 'Plan: CSMF → NSMF instantiate slice · NSSMF binds RAN/CN/TN subnets · UPF + SMF + MEC binding · NSSF + AMF NSSAI policy push · Snowflake-side audit log · acceptance test plan run', 'Decisioning'),
...
const ossTmf921Sla: SectionScenario = {
  ...
  subtitle: 'Proactive TMF 921 SLA management for 412 B2B accounts · slo_burnrate_v2 predicts 14 accounts will breach 99.95% in next 30 days · pre-emptive remediation prevents £220k SLA-credit (B2B 99.95% breach credits compound at 5–10% of MRC).',
  durationSec: 22,
  events: [
    { ...e(0, 'detect', 'TMF 921 SLA dashboard · slo_burnrate_v2 forecasts 14 of 412 B2B accounts will breach 99.95% in next 30 days · projected SLA-credit £220k', 'Decisioning', 'warn'), focus: { route: '/oss/slo', target: 'page' } },
    ...
    e(4, 'log', 'Top accounts at risk: Lloyds (£42k credit risk) · Barclays (£34k) · NHS Manchester (£22k) · TfL (£22k)', 'Decisioning'),
    ...
    { ...e(18, 'verify', 'Day-30: 13 of 14 retained inside SLA (1 unavoidable due to OR fault, customer-credit issued) · projected £198k SLA-credit avoided', 'Decisioning', 'success'), focus: { route: '/oss/slo', target: 'page' } },
